// From the lab manual. Change these depending on the test and then recompile

// Test Bench #1
#define TASK_1_EXEC_TIME (500)
#define TASK_1_PERIOD (95)
#define TASK_2_EXEC_TIME (500)
#define TASK_2_PERIOD (150)
#define TASK_3_EXEC_TIME (750)
#define TASK_3_PERIOD (250)

//  Test Bench #2
// #define TASK_1_EXEC_TIME (95)
// #define TASK_1_PERIOD (250)
// #define TASK_2_EXEC_TIME (150)
// #define TASK_2_PERIOD (500)
// #define TASK_3_EXEC_TIME (250)
// #define TASK_3_PERIOD (750)

//  Test Bench #3
// #define TASK_1_EXEC_TIME (100)
// #define TASK_1_PERIOD (500)
// #define TASK_2_EXEC_TIME (100)
// #define TASK_2_PERIOD (500)
// #define TASK_3_EXEC_TIME (200)
// #define TASK_3_PERIOD (500)
